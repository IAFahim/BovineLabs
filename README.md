...
You are absolutely right, and I appreciate you calling out that oversight. Using `ref` on a value that doesn't change is not only bad practice—it actively prevents Burst from applying alias optimizations, forces unnecessary pointer dereferencing, and creates confusing APIs.

Primitives (`float`, `int`) should just be passed by value. Large structs should use `in` (if read-only) to avoid stack copies, and `out` or `ref` for mutations/returns.

I have meticulously combed through the document to fix the Burst ABI rules and pure function examples so they reflect true high-performance C# standards. I've also incorporated the 24-frame delay caveat for `[TestLeakDetection]`.

Here is the fully unified, corrected, and finalized Elite Unity ECS Standards document.
...
> **Philosophy:** Zero cyclomatic complexity systems, strict zero-allocation hot paths, strict Burst ABI compliance (proper `in`/`ref`/`out` usage), and mathematical proof of stability through rigorous testing.
...
[assembly: InternalsVisibleTo("Feature.Authoring")][assembly: InternalsVisibleTo("Feature.Tests")]
```
...
```csharp[WorldSystemFilter(Worlds.Simulation)]
...
`Unity.Mathematics.Random` cannot be shared across threads. We use `GlobalRandom` (which maintains pre-allocated states per worker thread).
...
Structural changes (adding/removing components) stall the main thread, invalidate caches, and prevent parallel processing.

- **RULE:** Never use `EntityCommandBuffer.AddComponent<T>` for state toggles that happen frequently (`IsStunned`, `IsSelected`).
...
Enums are rigid. Strings are slow. `KSettings<T, TV>` converts human-readable strings from ScriptableObjects into Burst-compatible unmanaged values (`byte`, `int`).
...
- **`SettingsSingleton<T>`:** For data that must exist *before* ECS worlds spin up (Boot configs, Themes). Available instantly via `MyGlobalSettings.I`.
- **`SettingsBase`:** For ECS gameplay data. Use `[SettingsWorld("Client", "Server")]` to target specific network worlds.

---

## 9. `[Singleton]` Buffers (The Many-to-One Pattern)

When multiple subscenes or mods contribute to a single list of data, use `BovineLabs.Core.Settings.SingletonAttribute` to automatically merge them.

```csharp[Singleton]
public struct CraftableItemElement : IBufferElementData { public int ItemId; }
```
At runtime, `SingletonSystem` automatically finds all entities with this buffer, copies them into **one master singleton entity**, and triggers a one-frame `SingletonInitialize` flag.

---

## 10. Object Management: `IUID` and `ObjectDefinition`

Traditional ECS prefab passing breaks over networks. We use `ObjectId` (a deterministic integer) backed by `ObjectDefinition` assets.
...
**Zero Cyclomatic Complexity (CC = 1) in System `OnUpdate` methods.** A System is an orchestrator. It queries data, handles dependencies, schedules jobs, and returns. No `if`, `for`, or `foreach` logic loops.

### 11.1 The Pure Function & Burst ABI Rule
When logic must be executed inside a job, it should be extracted into **pure static methods**.
- **RULE:** If an input value is read-only and small (primitives like `int`, `float`, `float3`), pass by value.
- **RULE:** If an input value is a large struct (>16 bytes), pass by `in` to prevent stack copying and accidental mutation.
- **RULE:** Do not use `ref` for inputs that do not change. Using `ref` on read-only data defeats Burst's alias analysis and adds pointer dereferencing overhead.
- **RULE:** For struct outputs, pass via `out` or `ref`. Primitives can be returned directly.

```csharp
// ❌ BAD: Passing large structs by value causes stack copies. 
// Using 'ref' on variables that don't change disables Burst alias optimizations.
public DamageResult CalculateDamage(ref WeaponStats weapon, ArmorStats armor) 
{ 
    return new DamageResult { Value = weapon.Damage - armor.Defense }; 
}

// ✅ GOOD: Pure static method. 
// - 'in' used for read-only structs (no copying, prevents mutation).
// - 'out' used for struct outputs.
// - Primitives passed by value.
public static class CombatMath
{
    [BurstCompile]
    public static void CalculateMitigatedDamage(in WeaponStats weapon, in ArmorStats armor, float dt, out DamageResult result)
    {
        result = new DamageResult 
        { 
            FinalDamage = math.max(1f, weapon.Damage * (100f / (100f + armor.Defense))) * dt
        };
    }
}
```
...
### 12.1 `IJobForThread`
Divides work across a **known number of threads** instead of Unity's dynamic batching. Each worker thread receives a contiguous slice.
...
You cannot store `NativeHashMap`s directly on an Entity. BovineLabs solves this by reinterpreting `DynamicBuffer<byte>` as fully functional, zero-allocation HashMaps.

```csharp
// 1. Declaration[InternalBufferCapacity(0)]
...
Bypasses Unity's parallel job safety checks. Use when you can mathematically guarantee no two threads touch the same Entity ID. Requires manual dependency injection.
...
Instead of boilerplate-heavy `IAspect`s, define a `partial struct` that implements `IFacet`. The Roslyn Source Generator (`BovineLabs.FacetGenerator`) writes the `Lookup`, `TypeHandle`, and `ResolvedChunk` logic for you.
...
Abstracts `EntityManager`, `EntityCommandBuffer`, `ParallelWriter`, and `IBaker` into a single interface so composition logic is written exactly once.
...
When `DestroyEntity` is enabled, `DestroyOnDestroySystem` recursively traverses `LinkedEntityGroup`s, enabling destruction on all child entities automatically, preventing orphaned memory leaks.
...
- **Runtime Toggling:** Toggle the `LoadSubScene` enableable component to load/unload scenes dynamically at runtime. Never use `SceneSystem.LoadSceneAsync` directly in logic.

---

## 19. High-Performance Physics States

Unity Physics `CollisionEvent` is stateless. `BovineLabs.Core.PhysicsStates` is a Burst-compiled, fully parallelized system that converts these into `Enter`, `Stay`, and `Exit` states automatically.

Attach `StatefulCollisionEventAuthoring` to colliders, then query `DynamicBuffer<StatefulCollisionEvent>` in your systems.
...
Prevents the server from sending every ghost to every client. Attach `InputBounds` to the player connection entity. The `RelevancySystem` spatially hashes the bounds and only serializes ghosts that fall inside the client's localized grid.

---

## 21. Deterministic Pause System

Disabling systems to "pause" causes `SystemAPI.Time.ElapsedTime` to drift, creating massive catch-up lag spikes in the `FixedStepSimulationSystemGroup` when unpaused.

`BovineLabs.Core.Pause` intercepts the `IRateManager`. When `PauseGame` is enabled, it literally freezes ECS time progression.
...
2. Must include `[assembly: DisableAutoCreation]` in `AssemblyInfo.cs` to prevent test systems from corrupting the live `SimulationSystemGroup`.
3. **Always** inherit tests from `BovineLabs.Testing.ECSTestsFixture`. This provides a pristine isolated `World` and forces `JobsUtility.JobDebuggerEnabled = true` to catch race conditions.

---

## 24. Memory Leak Detection (The 24-Frame Rule)

Creating a `NativeArray` with `Allocator.TempJob` without `.Dispose()` causes a leak. `[TestLeakDetection]` automatically checks for leaks at the end of the test.

⚠️ **WARNING ON FRAME DELAYS:** The `[TestLeakDetection]` attribute takes approximately **24 frames** of ECS lifecycle execution to fully flush and validate internal allocators in some Unity versions.
- **DO NOT** use this attribute on tests that rely on strict, frame-by-frame time stepping, or those measuring exact `SystemAPI.Time` progression across updates.
- **USE IT** strategically on isolated job tests, pure structural/allocation tests, or setup/teardown validation where timing drift is irrelevant.

```csharp
[Test][TestLeakDetection] // Catches leaks, but disrupts exact frame-time stepping
...
As detailed in Rule 11.1, do not use `ref` for variables that don't change.
...
{
    [BurstCompile]
    public static void CalculateMitigatedDamage(in WeaponStats weapon, in ArmorStats armor, out DamageResult result)
    {
        result = new DamageResult 
        { 
            FinalDamage = math.max(1f, weapon.Damage * (100f / (100f + armor.Defense))) 
        };
    }
}
...
{
    [Test]
    public void CalculateMitigatedDamage_ReturnsCorrectValues()
    {
        var weapon = new WeaponStats { Damage = 100f };
        var armor = new ArmorStats { Defense = 100f };
        
        CombatMath.CalculateMitigatedDamage(in weapon, in armor, out var result);
...
Use `.Run()` instead of `.Schedule()` to execute jobs synchronously on the main thread for the test, bypassing the System orchestrator overhead.
...
```
*Note: If the system uses an `EntityCommandBuffer`, you must manually `Update` the `EndSimulationEntityCommandBufferSystem` to trigger playback before asserting.*
...
Blob assets require unmanaged allocation. Ensure you use the `BlobBuilder` and `Dispose()` of the reference properly to avoid catastrophic leaks.

```csharp
[Test][TestLeakDetection]
...
Core mathematical systems, tight loops, and custom collections MUST have performance benchmarks via `Unity.PerformanceTesting`.
...
3. [ ] **Burst ABI Compliance:** Are pure functions written as `static` methods using `in`/`out` instead of returning structs, and completely avoiding `ref` for unmodified data?
4. [ ] **No Managed Mocks:** Are you instantiating real `NativeArray`s and `Entity` components instead of using mocking frameworks?
...
7.[ ] **ECB Syncing:** If your system uses `EntityCommandBuffer`, did you explicitly update the ECB system to force playback?
